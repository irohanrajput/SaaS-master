...
import { LinkedInMetrics, TimeframeType } from './types'
import { filterPostsByTimeframe, filterGrowthDataByTimeframe, formatNumber } from './utils'

interface LinkedInDashboardProps {
  data: LinkedInMetrics | null
  timeframe: TimeframeType
  loading: boolean
}

export default function LinkedInDashboard({ data, timeframe, loading }: LinkedInDashboardProps) {
  if (loading) {
    return (
      <div className="flex items-center justify-center py-32">
        <div className="flex flex-col items-center">
          <Loader2 className="w-12 h-12 text-blue-600 animate-spin" />
          <p className="mt-4 text-gray-600 font-medium">Loading LinkedIn metrics...</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <Card>
        <CardContent className="py-12 text-center">
          <p className="text-gray-500">{data?.reason || 'No LinkedIn data available'}</p>
        </CardContent>
      </Card>
    )
  }
        ...
        <CardContent>
          <div className="flex items-center justify-between">
            ...
            <div className="space-y-2 text-sm">
              ...
              {data.engagementScore?.clicks !== undefined && (
                <div className="flex justify-between gap-8">
                  <span className="text-gray-600">Clicks:</span>
                  <span className="font-semibold">{formatNumber(data.engagementScore.clicks)}</span>
                </div>
              )}
            </div>
          </div>
        ...
        <CardContent>
          <div className="space-y-2">
            ...
            <div>
              ...
              <p className="text-2xl font-bold">{formatNumber(data.companyFollowers || 0)}</p>
            </div>
            <div>
              <p className="text-sm text-gray-500">Posts Analyzed</p>
              <p className="font-semibold">{data.scrapedPostsCount || 0}</p>
            </div>
          </div>
        </CardContent>
      </Card>
